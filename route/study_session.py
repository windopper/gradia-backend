...
async def get_all_study_sessions(
    current_user = Depends(get_current_user_from_backend_token),
    subject_id: Optional[str] = Query(None, description="특정 과목 ID에 대한 학습 세션만 조회")
):
    """
    ...
    try:
        ...
        if subject_id:
            sessions = await get_study_sessions_by_subject_id(user_id, subject_id)
        else:
            sessions = await get_study_sessions_by_user_id(user_id)
...
async def get_study_session(
    ...
    """
    try:
        session = await get_study_session_by_id(session_id)
...
async def update_existing_study_session(
    session_id: str,
    ...
    try:
        ...
        # 업데이트 수행
        updated_session = await update_study_session(session_id, update_dict)
...
async def delete_existing_study_session(
    session_id: str,
    current_user = Depends(get_current_user_from_backend_token)
):
    """
    ...
    try:
        ...
        # 삭제 수행
        success = await delete_study_session(session_id)
